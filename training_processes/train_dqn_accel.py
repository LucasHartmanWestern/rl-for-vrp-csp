...
def train_dqn(ev_info, metrics_base_path, experiment_number, chargers, environment, routes, date, action_dim, global_weights, aggregation_num, zone_index,
    ...
    if rank > 0:
        ...
        if agent_by_zone:  # Use same NN for each zone
            ...
            comm.bcast('initializing_dqn', root=0)
            comm.bcast((state_dimension,action_dim, layers, device, learning_rate), root=0)
        ...
        for i in range(num_episodes):  # For each episode
            ...
            while not sim_done:  # Keep going until every EV reaches its destination
                ...
                for car_idx in range(num_cars): # For each car
                    ...
                    if random_threshold[i, car_idx] < epsilon:  # Epsilon-greedy action selection
                        if nn_by_zone:
            ...
            if MP_CHANGE:
                ...
                if len(buffers[0]) >= batch_size: # Buffer is full enough
                    for agent_ind in range(num_cars):
                        ...
                        if agent_by_zone:
                            ...
                            #                                           target_q_networks[agent_ind], optimizers[agent_ind], device, dqn_rng, batch_size))
...
def train_multi_agent(agent_exp, discount_factor, q_network, target_q_network, optimizer, device, dqn_rng, batch_size):
    mini_batch = dqn_rng.choice(np.array([Experience(exp.state.cpu().numpy(), exp.distribution, exp.reward, exp.next_state.cpu().numpy(), exp.done) if isinstance(exp.state, torch.Tensor) else exp for exp in agent_exp], dtype=object), batch_size, replace=False)
    experiences = map(np.stack, zip(*mini_batch))  # Format experiences

    agent_learn(experiences, discount_factor, q_network, target_q_network, optimizer, device)

def dqn_worker():
    while True:
        step = comm.bcast(None,root=0)
        if step == 'initializing_dqn':
            state_dimension,action_dim, layers, device, learning_rate = comm.bcast(None,root=0)
            q_network, target_q_network = initialize(state_dimension, action_dim, layers, device)
            optimizer = optim.RMSprop(q_network.parameters(), lr=learning_rate)  # Use RMSprop optimizer
