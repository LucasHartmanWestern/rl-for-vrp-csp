...
def train_denser(ev_info, 
    ...
    state_dimension = (c['environment_settings']['num_of_chargers']* 3 * 2) + 6
    ...
    for agent_idx in range(num_agents):
        ...
        agent = DenserAgent(state_dimension, action_dim, num_cars, seed, agent_idx,\
    ...
    max_generation  = denser_agents_list[0].max_generation
    population_size = denser_agents_list[0].population_size

    # processes = []
    # comms = []
    # # Initialize workers environment reate a setup and environment for each population
    ...
    #     # else:
    #     #     agents_by_pop = [denser_agents_list[car_idx].population[pop_idx] for car_idx in range(num_cars)]
        
        
    #     parent_comm, child_comm = mp.Pipe() 
    #     cumulative_flag = 'average_rewards_when_training' in nn_c and nn_c['average_rewards_when_training']
    #     p = mp.Process(target=worker_environment, args=(child_comm, pop_idx, agents_by_pop,
    #                    copy.deepcopy(environment), chargers, routes, unique_chargers,
    #                    num_agents, num_cars, agent_by_zone, cumulative_flag, dtype, device))
    #     processes.append(p)
    ...
    # Start running workers environment for population
    for comm in workers_comm:
        comm.send("init_environment")
        comm.send((routes, chargers, aggregation_num))
        
    fitnesses = torch.full((population_size, num_agents), float('-inf'), device=device)
    
    for generation in range(max_generation):
        # Start the run of one environment episode
        for pop_idx, comm in enumerate(workers_comm):
            if agent_by_zone:
                agents_by_pop = [denser_agents_list[0].population[pop_idx]]
            else:
                agents_by_pop = [denser_agents_list[car_idx].population[pop_idx] for car_idx in range(num_cars)]
            comm.send("run_episode")
            comm.send(agents_by_pop)

        # receive the results from one run episode
        for comm in workers_comm:
            pop_idx, cumulative = comm.recv()
            if agent_by_zone:
        ...
        # --- Evaluate the population with the best fitness for logging & metrics ---
        idx_best_pop = torch.argmax(fitnesses.mean(axis=1))
        best_fitness_reward = fitnesses[idx_best_pop,:].mean()
        avg_rewards.append((best_fitness_reward, aggregation_num, zone_index, main_seed))

        workers_comm[idx_best_pop].send("get_metrics")
        metrics_best_ep = workers_comm[idx_best_pop].recv() 

        for met in metrics_best_ep:
            metrics.append(met)
        metri = None
    ...
    #         agent.tell(fitnesses[:, agent_idx].flatten())
        ...
        if verbose:
            ...
            print_log(
                ...
                f'Generation: {generation+1}/{max_generation}) - '
                f'avg reward {best_fitness_reward:.3f}',
                date, elapsed_time
            )

        # periodic save
        if ((generation + 1) % eps_per_save == 0 and generation > 0 and train_model) \
           or (generation == max_generation - 2):
        ...
        if best_fitness_reward > best_avg:
            best_avg = best_fitness_reward.float().cpu()
            for comm in workers_comm:
                comm.send("best_avg")
                comm.send(best_avg)
            ...
            if verbose:
                print_log(
                    f'Zone: {zone_index+1} Gen: {generation+1}/{max_generation} '
    ...
    # End of evolution
    print(f'Rewards for population evolution: {best_fitness_reward:.3f} '
          f'after {max_generation} generations')
