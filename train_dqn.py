...
from dqn_agent import initialize, agent_learn, get_actions, soft_update, save_model
...
def train_dqn(chargers, environment, routes, date, action_dim, global_weights, aggregation_num,\
    ...
    random_threshold = dqn_rng.random((num_episodes, environment.num_cars))
    # Initialize replay buffer with fixed size
    buffers = [deque(maxlen=buffer_limit) for _ in range(environment.num_cars)]  
    ...
    for episode in range(num_episodes):  # For each episode
        ...
        for car_idx in range(environment.num_cars): # For each agent
            ...
            action_values = get_actions(state, q_networks, random_threshold, epsilon, episode, car_idx,\
                                        device, agent_by_zone)  # Get the action values from the agent

            t2 = time.time()

            distribution = action_values.detach().numpy()  # Convert PyTorch tensor to NumPy array
            distributions_unmodified.append(distribution.tolist())  # Track outputs before the sigmoid 
        ...
        for d in range(len(distributions_unmodified)):
            ...
            buffers[d].append(experience(states[d], distributions_unmodified[d], rewards[d], \
                                         states[(d + 1) % max(1, (len(distributions_unmodified) - 1))], done))  
        ...
        for agent_ind in range(environment.num_cars):
            ...
            if len(buffers[agent_ind]) >= batch_size: # Buffer is full enough
                ...
                mini_batch = dqn_rng.choice(np.array(buffers[agent_ind], dtype=object), batch_size,\
                                            replace=False)
    ...
    weights_list = [q_network.cpu().state_dict() for q_network in q_networks]
