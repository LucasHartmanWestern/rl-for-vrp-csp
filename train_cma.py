    ...
    for generation in range(cma_info.max_generation):
        ...
        for pop_idx in range(population_size):
            ...
            for car_idx in range(num_cars):
                state = environment.reset_agent(car_idx)  # Reset environment for the car
                ...
                environment.generate_paths(car_route, None)  # Stack the generated paths in the environment
        ...
        environment.simulate_routes()  # Simulate the environment with the best solutions
        _, _, _, _, rewards = environment.get_results()  # Get the resulting rewards
        avg_reward = rewards.mean()  # Calculate average reward for this generation
...
def save_paths(fname, best_paths):
    """
    Saves the best paths to a file.

    Parameters:
        fname (str): The file name to save the paths to.
        best_paths (list): The best paths found during training.
    """
    folder_path = 'outputs/best_paths'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    np.save(f'{folder_path}/{fname}', np.array(best_paths, dtype=object))
