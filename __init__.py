    ...
    for session in range(num_training_sesssions):
        ...
        for agent in range(num_of_agents):
            ...
            if starting_charge is None:
                ...
                starting_charge[agent] += 1000 * random.randrange(0, 10)  # 5%

        elapsed_time = time.time() - start_time
        start_time = time.time()
        print(f"Calcualte Starting Charges: - {int(elapsed_time // 3600)}h, {int((elapsed_time % 3600) // 60)}m, {int(elapsed_time % 60)}s")
        ...
        elapsed_time = time.time() - start_time
        start_time = time.time()
        print(f"Build Routes: - {int(elapsed_time // 3600)}h, {int((elapsed_time % 3600) // 60)}m, {int(elapsed_time % 60)}s")
        ...
        if train_model:
            ...
            if algorithm == "DQN":
                ...
                train_dqn(env, seeds, epsilon, epsilon_decay, discount_factor, num_episodes, batch_size, buffer_limit, state_dimension, action_dimension, num_of_agents, start_from_previous_session, layers, fixed_attributes)
            else:
        ...
        if save_data:
            env.write_path_to_csv(f'outputs/routes_{num_of_agents}_{num_episodes}_{seeds}.csv')
            env.write_chargers_to_csv(f'outputs/chargers_{num_of_agents}_{num_episodes}_{seeds}.csv')
            env.write_reward_graph_to_csv(f'outputs/rewards_{num_of_agents}_{num_episodes}_{seeds}.csv')
            env.write_charger_traffic_to_csv(f'outputs/traffic_{num_of_agents}_{num_episodes}_{seeds}.csv')

        if generate_plots:
            route_data = read_csv_data(f'outputs/routes_{num_of_agents}_{num_episodes}_{seeds}.csv')
            charger_data = read_csv_data(f'outputs/chargers_{num_of_agents}_{num_episodes}_{seeds}.csv')
            reward_data = read_csv_data(f'outputs/rewards_{num_of_agents}_{num_episodes}_{seeds}.csv')
            traffic_data = read_csv_data(f'outputs/traffic_{num_of_agents}_{num_episodes}_{seeds}.csv')
