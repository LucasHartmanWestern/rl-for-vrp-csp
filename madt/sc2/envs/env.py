...
def make_eval_env(all_args, merl_env, action_dim, n_threads=1):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "StarCraft2":
                ...
                env = Merl_Env_Wrapper(merl_env, action_dim)
            else:
...
class Env:
    def __init__(self, merl_env, action_dim, n_threads=1):
        parser = get_config()
        all_args = parser.parse_known_args()[0]
        self.real_env = make_eval_env(all_args, merl_env, action_dim, n_threads)
